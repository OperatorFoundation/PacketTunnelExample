...
class UDPServerConnection: Connection {
    ...
    var responseSource: DispatchSource?//dispatch_source_t?
    ...
    func getEndpointFromSocketAddress(socketAddressPointer: UnsafePointer<sockaddr>) -> (host: String, port: Int)? {
		...
		switch Int32(socketAddress.sa_family) {
			...
			case AF_INET6:
                ...
                var socketAddressInet6 = UnsafeRawPointer(socketAddressPointer).load(as: sockaddr_in.self)
				let length = Int(INET6_ADDRSTRLEN) + 2
				var buffer = [CChar](repeating: 0, count: length)
				let hostCString = inet_ntop(AF_INET6, &socketAddressInet6.sin_addr, &buffer, socklen_t(length))
				let port = Int(UInt16(socketAddressInet6.sin_port).byteSwapped)
                return (String(cString:hostCString!), port)
    ...
    func createSocketWithAddressFamilyFromAddress(address: String) -> Bool {
		...
		//	return false
		//}
        guard let newResponseSource:DispatchSource = (DispatchSource.makeReadSource(fileDescriptor: newSocket, queue: DispatchQueue.main) as? DispatchSource) else {
            close(newSocket)
            return false
        }
		...
		newResponseSource.setCancelHandler() {
			...
			let UDPSocket = Int32((newResponseSource as DispatchSourceProcess).handle)
			close(UDPSocket)
		}
		...
		newResponseSource.setEventHandler() {
			...
			let UDPSocket = Int32((source as DispatchSourceProcess).handle)
            ...
            guard let endpoint = withUnsafePointer(to: &socketAddress, {
                $0.withMemoryRebound(to:sockaddr.self, capacity:1) {
                    self.getEndpointFromSocketAddress(socketAddressPointer: $0) }}) else {
                        simpleTunnelLog("Failed to get the address and port from the socket address received from recvfrom")
                        self.closeConnection(.all)
                        return
            }

			let responseDatagram = Data(bytes: response)
		...
		switch addressFamily {
			case AF_INET:
				...
				sent = withUnsafePointer(to: &serverAddress.sin) {
					sendto(UDPSocket, rawPtr, data.count, 0, UnsafeMutableRawPointer(mutating: $0).bindMemory(to: sockaddr.self, capacity: 1), socklen_t(serverAddress.sin.sin_len))
				}
			...
			case AF_INET6:
                ...
                let rawPtr = UnsafeRawPointer(uint8Ptr)
                sent = withUnsafePointer(to: &serverAddress.sin6) {
                    sendto(UDPSocket, rawPtr, uint8PtrCount, 0, UnsafeMutableRawPointer(mutating: $0).bindMemory(to: sockaddr.self, capacity: 1), socklen_t(serverAddress.sin6.sin6_len))
            }
