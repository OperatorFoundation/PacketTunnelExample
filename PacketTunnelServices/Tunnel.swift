...
open class Tunnel: NSObject
{
	...
	func serializeMessage(_ messageProperties: [String: AnyObject]) -> Data? {
		...
		do {
            ...
            //resultData = Data(capacity: Int(totalLength))
            //resultData?.append(totalLength, count: MemoryLayout<UInt32>.size)
            //let buffer = UnsafeBufferPointer(start: &totalLength, count: 1)
        ...
        resultData = Data(bytes: (messageData?.bytes)!, count:(messageData?.length)!)
		return resultData 
	}
	...
	func handlePacket(_ packetData: Data) -> Bool
    {
		...
		guard let command = properties[TunnelMessageKey.Command.rawValue] as? Int
        else
        {
		...
		guard let commandType = TunnelCommand(rawValue: command)
        else
        {
        ...
        {
			case .data:
				guard let data = properties[TunnelMessageKey.Data.rawValue] as? Data
                else { break }
